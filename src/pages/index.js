...
import { increaseAnimationCounter } from '../components/homePage/state/actions/action'

const IndexPage = ({ data, animationCounter, increaseAnimationCounter}) =>{

  useEffect(()=>{
    let t1 = gsap.timeline({
      onComplete:()=>{
        increaseAnimationCounter(animationCounter)
      }
    })
    ...
    if (animationCounter === 0){
      ...
      t1.set(".overlay-container",{display: 'none'})
      t1.set(".intro-overlay-container", { display: 'none' })
    }
...
const mapStateToProps=(state)=>{
  console.log(state.animationCounter.homeOverlayAnimationCounter)
  return {
    animationCounter: state.animationCounter.homeOverlayAnimationCounter
  }
}

const mapDispatchToProps = (dispatch)=>{
  return {
    increaseAnimationCounter:(counter)=>{
      dispatch(increaseAnimationCounter(counter))
    }
  }
}
